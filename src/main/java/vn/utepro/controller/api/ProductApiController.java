...
public class ProductApiController {
	...
	public ResponseEntity<?> getProduct(@RequestParam("id") Long id) {
		...
		return p.map(prod -> new ResponseEntity<>(new Response(true, "Thành công", prod), HttpStatus.OK)).orElseGet(
				() -> new ResponseEntity<>(new Response(false, "Không tìm thấy Product", null), HttpStatus.NOT_FOUND));
	}
	...
	public ResponseEntity<?> addProduct(@RequestParam("productName") String productName,
			...
			@RequestParam("unitPrice") Double unitPrice, @RequestParam("discount") Double discount,
		...
		if (existing.isPresent()) {
			return new ResponseEntity<>(new Response(false, "Sản phẩm này đã tồn tại", existing.get()),
		...
		try {
			...
			e.printStackTrace();
			return new ResponseEntity<>(new Response(false, "Lỗi: " + e.getMessage(), null),
